    ...
    <div class="text-center">
        <v-form>
          <v-container>
            ...
            <v-row class="w-cont-center">
              <v-col cols="12" md="12">
                <mdb-input type="textarea" v-model="data.qualitification" label="우대사항" :rows="5" />
              </v-col>
            </v-row>

            <v-row class="w-cont-center">
              <v-col cols="12" md="12">
                <mdb-input type="textarea" v-model="data.content" label="모집내용" :rows="5" />
              </v-col>
            </v-row>
          </v-container>
        </v-form>
...
export default({
  ...
  methods:{
    ...
    Write() {
      ...
      console.log('user_name:'+this.user_name)
          axios.post('/recruits/create/',{
              user_name: this.user_name,
              title: this.data.title,
              tech_category: this.data.tech_category,
              end_date: this.data.end_date,
              region: this.data.region,
              qualitificatioin: this.data.qualitification,
      ...
      //form에선 용어, 백으로 보낼땐 숫자로
       var result = []
        for (let i = 0; i< this.data.tech_category.length; i++) {
          ...
          axios.put('/recruits/'+this.post_id+'/update/',{
    ...
    getRecruit() {
                axios.get('/recruits/'+this.$route.params.id)
                    .then(res => {
                        ...
                        this.data.qualitification= res.data.qualitificatioin
  ...
  mounted() {
            if(this.$route.params.id!==undefined) this.getRecruit()
        }
})
